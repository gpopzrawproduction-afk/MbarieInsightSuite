...
public class EmailMessageConfiguration : IEntityTypeConfiguration<EmailMessage>
{
    public void Configure(EntityTypeBuilder<EmailMessage> builder)
    {
        ...
        var extractedKeywordsProperty = builder.Property(e => e.ExtractedKeywords)
            .HasConversion(
                v => string.Join(";", v),
                v => v.Split(';', StringSplitOptions.RemoveEmptyEntries).ToList())
            .HasMaxLength(2000);
        extractedKeywordsProperty.Metadata.SetValueComparer(ValueComparerFactory.StringListComparer);

        var actionItemsProperty = builder.Property(e => e.ActionItems)
            .HasConversion(
                v => string.Join("|||", v),
                v => v.Split("|||", StringSplitOptions.RemoveEmptyEntries).ToList())
            .HasMaxLength(4000);
        actionItemsProperty.Metadata.SetValueComparer(ValueComparerFactory.StringListComparer);
