...
public class AssetMonitorConfiguration : IEntityTypeConfiguration<AssetMonitor>
{
    public void Configure(EntityTypeBuilder<AssetMonitor> builder)
    {
        ...
        var specificationsProperty = builder.Property(x => x.Specifications)
            .HasConversion(
                v => JsonSerializer.Serialize(v, (JsonSerializerOptions?)null),
                v => JsonSerializer.Deserialize<Dictionary<string, string>>(v, (JsonSerializerOptions?)null) ?? new Dictionary<string, string>())
            .HasColumnType("TEXT");
        specificationsProperty.Metadata.SetValueComparer(ValueComparerFactory.StringDictionaryComparer);

        var associatedMetricsProperty = builder.Property(x => x.AssociatedMetrics)
            .HasConversion(
                v => JsonSerializer.Serialize(v, (JsonSerializerOptions?)null),
                v => JsonSerializer.Deserialize<List<string>>(v, (JsonSerializerOptions?)null) ?? new List<string>())
            .HasColumnType("TEXT");
        associatedMetricsProperty.Metadata.SetValueComparer(ValueComparerFactory.StringListComparer);
