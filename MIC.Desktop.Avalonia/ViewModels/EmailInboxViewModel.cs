...
public class EmailInboxViewModel : ViewModelBase
{
    ...
    public EmailInboxViewModel(
        ...
        IEmailOAuth2Service? gmailOAuthService = null,
        IEmailOAuth2Service? outlookOAuthService = null,
        IErrorHandlingService? errorHandlingService = null,
        INotificationService? notificationService = null)
    {
        var provider = Program.ServiceProvider;

        _mediator = mediator ?? provider?.GetService<IMediator>();
        _emailSyncService = emailSyncService ?? provider?.GetService<IEmailSyncService>();
        _emailAccountRepository = emailAccountRepository ?? provider?.GetService<IEmailAccountRepository>();
        _emailRepository = emailRepository ?? provider?.GetService<IEmailRepository>();
        _gmailOAuthService = gmailOAuthService ?? provider?.GetKeyedService<IEmailOAuth2Service>("Gmail");
        _outlookOAuthService = outlookOAuthService ?? provider?.GetKeyedService<IEmailOAuth2Service>("Outlook");
        _errorHandlingService = errorHandlingService ?? provider?.GetRequiredService<IErrorHandlingService>()
            ?? throw new InvalidOperationException("Error handling service is not configured.");
    ...
    private async Task LoadEmailsAsync()
    {
        ...
        {
            _errorHandlingService.HandleException(ex, "Load Emails");
    ...
    private async Task SyncEmailsAsync()
    {
        if (_emailSyncService == null)
        {
            ...
            _notificationService.ShowError("Email sync service is not available.", category: "Email Sync");
            return;
        }
        ...
        if (_emailAccountRepository == null)
        {
            ...
            _notificationService.ShowError("Email account repository is not available.", category: "Email Sync");
            return;
        }
        ...
        try
        {
            ...
            if (userId == Guid.Empty)
            {
                _notificationService.ShowError("User not logged in", category: "Email Sync");
            ...
            if (accounts.Count == 0)
            {
                ...
                _notificationService.ShowInfo("No email accounts connected.", category: "Email Sync");
                return;
            }
            ...
            foreach (var account in accounts)
            {
                ...
                if (!result.Success)
                {
                    _notificationService.ShowError(result.ErrorMessage ?? "Email sync failed.", category: "Email Sync");
                    SyncStatus = "Sync failed";
                    return;
                }
            ...
            _notificationService.ShowSuccess(SyncStatus, category: "Email Sync");
        ...
        {
            SyncStatus = "Sync failed";
            _notificationService.ShowError($"Email sync failed: {ex.Message}", category: "Email Sync");
            _errorHandlingService.HandleException(ex, "Email Sync");
        }
        finally
        {
            IsSyncing = false;
    ...
    private async Task MarkAsReadAsync(EmailDto email)
    {
        ...
        {
            _notificationService.ShowError("Email repository is not available.", category: "Email");
            return;
        }
        ...
        if (entity == null)
        {
            _notificationService.ShowError("Email not found.", category: "Email");
            return;
        }
        ...
        _notificationService.ShowInfo($"Marked as read: {email.Subject}", category: "Email");
        await LoadEmailsAsync();
    }
    ...
    private async Task ToggleFlagAsync(EmailDto email)
    {
        ...
        _notificationService.ShowInfo($"Flag toggled: {email.Subject}", category: "Email");
        await LoadEmailsAsync();
    }
    ...
    private async Task ArchiveEmailAsync(EmailDto email)
    {
        ...
        _notificationService.ShowSuccess($"Archived: {email.Subject}", category: "Email");
        await LoadEmailsAsync();
    }
    ...
    private async Task DeleteEmailAsync(EmailDto email)
    {
        ...
        _notificationService.ShowInfo($"Deleted: {email.Subject}", category: "Email");
        await LoadEmailsAsync();
    }
    ...
    private async Task AddGmailAccountAsync()
    {
        try
        {
            ...
            {
                _notificationService.ShowError("Gmail OAuth service is not available.", category: "Email Accounts");
                return;
            }
            ...
            if (userId == null)
            {
                _notificationService.ShowError("User not logged in", category: "Email Accounts");
                return;
            }
            ...
            if (_mediator == null)
            {
                _notificationService.ShowError("Mediator service is not available.", category: "Email Accounts");
                return;
            }
        ...
        {
            _errorHandlingService.HandleException(ex, "Add Gmail Account");
            _notificationService.ShowError("Failed to connect Gmail account", category: "Email Accounts");
        }
        finally
        {
            IsLoading = false;
    ...
    private async Task AddOutlookAccountAsync()
    {
        try
        {
            ...
            {
                _notificationService.ShowError("Outlook OAuth service is not available.", category: "Email Accounts");
                return;
            }
        ...
        {
            _errorHandlingService.HandleException(ex, "Add Outlook Account");
            _notificationService.ShowError("Failed to connect Outlook account", category: "Email Accounts");
        }
        finally
        {
            IsLoading = false;
    ...
    private async Task AddDirectEmailAccountAsync()
    {
        try
        {
            ...
            if (mainWindow == null)
            {
                NotificationService.Instance.ShowError("Cannot show dialog: Main window is null", category: "Email Accounts");
                return;
            }
            ...
            if (userId == null)
            {
                NotificationService.Instance.ShowError("User not logged in", category: "Email Accounts");
                return;
            }
            ...
            {
                NotificationService.Instance.ShowError("Email address is required", category: "Email Accounts");
                return;
            }
            
            if (string.IsNullOrWhiteSpace(result.Password))
            {
                NotificationService.Instance.ShowError("Password is required", category: "Email Accounts");
                return;
            }
            
            if (string.IsNullOrWhiteSpace(result.ImapServer))
            {
                NotificationService.Instance.ShowError("IMAP server is required", category: "Email Accounts");
                return;
            }
            
            if (string.IsNullOrWhiteSpace(result.SmtpServer))
            {
                NotificationService.Instance.ShowError("SMTP server is required", category: "Email Accounts");
                return;
            }
            ...
            {
                NotificationService.Instance.ShowError("Invalid IMAP port (must be 1-65535)", category: "Email Accounts");
                return;
            }
            
            if (result.SmtpPort < 1 || result.SmtpPort > 65535)
            {
                NotificationService.Instance.ShowError("Invalid SMTP port (must be 1-65535)", category: "Email Accounts");
                return;
            }
            ...
            if (_mediator == null)
            {
                NotificationService.Instance.ShowError("Mediator service is not available.", category: "Email Accounts");
                return;
            }
        ...
        {
            _errorHandlingService.HandleException(ex, "Add Email Account - Format Error");
            NotificationService.Instance.ShowError($"Invalid input format: {ex.Message}. Please check all fields.", category: "Email Accounts");
        }
        catch (Exception ex)
        {
            _errorHandlingService.HandleException(ex, "Add Email Account");
            NotificationService.Instance.ShowError($"Failed to add email account: {ex.Message}", category: "Email Accounts");
        }
        finally
        {
            IsLoading = false;
    ...
    {
        NotificationService.Instance.ShowInfo("Email account connection will open OAuth flow. Coming soon!", category: "Email Accounts");
    }

    private async Task ExportEmailsAsync()
    {
        NotificationService.Instance.ShowInfo("Exporting emails...", category: "Email");
    ...
    private async Task ComposeEmailAsync()
    {
        try
        {
            ...
            if (result == true)
            {
                NotificationService.Instance.ShowSuccess("Email sent successfully!", category: "Email");
            }
        }
        catch (Exception ex)
        {
            _errorHandlingService.HandleException(ex, "Compose Email");
        }
    }
    ...
    private async Task ReplyToEmailAsync(EmailDto email)
    {
        ...
        try
        {
            ...
            if (result == true)
            {
                NotificationService.Instance.ShowSuccess("Reply sent successfully!", category: "Email");
            }
        }
        catch (Exception ex)
        {
            _errorHandlingService.HandleException(ex, "Reply to Email");
        }
    }
    ...
    private async Task ForwardEmailAsync(EmailDto email)
    {
        ...
        try
        {
            ...
            if (result == true)
            {
                NotificationService.Instance.ShowSuccess("Email forwarded successfully!", category: "Email");
            }
        }
        catch (Exception ex)
        {
            _errorHandlingService.HandleException(ex, "Forward Email");
        }
    }
