...
public class DecisionContextConfiguration : IEntityTypeConfiguration<DecisionContext>
{
    public void Configure(EntityTypeBuilder<DecisionContext> builder)
    {
        ...
        var contextDataProperty = builder.Property(x => x.ContextData)
            .HasConversion(
                v => JsonSerializer.Serialize(v, (JsonSerializerOptions?)null),
                v => JsonSerializer.Deserialize<Dictionary<string, object>>(v, (JsonSerializerOptions?)null) ?? new Dictionary<string, object>())
            .HasColumnType("TEXT");
        contextDataProperty.Metadata.SetValueComparer(ValueComparerFactory.ObjectDictionaryComparer);

        var consideredOptionsProperty = builder.Property(x => x.ConsideredOptions)
            .HasConversion(
                v => JsonSerializer.Serialize(v, (JsonSerializerOptions?)null),
                v => JsonSerializer.Deserialize<List<string>>(v, (JsonSerializerOptions?)null) ?? new List<string>())
            .HasColumnType("TEXT");
        consideredOptionsProperty.Metadata.SetValueComparer(ValueComparerFactory.StringListComparer);
